...
const Node: React.FC<Props> = ({ node, expanded, toggleNodeExpanded }) => {
  return (
    <AccordionRoot
      elevation={3}
      expanded={expanded}
      onChange={() => toggleNodeExpanded ? toggleNodeExpanded(node) : null}
    >
      ...
      <AccordionDetails>
        {node?.blocks?.length ? <Block blocks={node?.blocks} /> :
          <Typography>This node doesn't have blocks.</Typography>}
      </AccordionDetails>
    </AccordionRoot>
  );
};
